...
def run_dataframe_comparison(df1,
    ...
    if columns_to_compare is not None:
        ...
        dropped_cols_df1 = [i for i in df1.columns if i not in common_columns]
        dropped_cols_df2 = [i for i in df2.columns if i not in common_columns]

        logging.info(f'Columns dropped from df1 because they are not in df2 {dropped_cols_df1}')
        logging.info(f'Columns dropped from df2 because they are not in df1 {dropped_cols_df2}')
    ...
    float_cols1 = [col for col in df1.columns if any(df1[col].dtype == i for i in ('float32','float64'))]
    float_cols2 = [col for col in df2.columns if any(df1[col].dtype == i for i in ('float32','float64'))]

    common_float_cols = list(set(float_cols1) & set(float_cols2))

    if columns_to_compare is not None:
        common_float_cols = [i for i in common_float_cols if i in columns_to_compare]
    ...
    common_int_cols = [col for col in df1.columns if df1[col].dtype == int]
